...
function orbisius_simple_feedback_init() {
    ...
    wp_register_style('simple_feedback', plugins_url("/assets/main{$suffix}.css", __FILE__), false,
            filemtime( plugin_dir_path( __FILE__ ) . "/assets/main{$suffix}.css" ) );
    wp_enqueue_style('simple_feedback');

    wp_enqueue_script( 'jquery' );
    wp_register_script( 'simple_feedback', plugins_url("/assets/main{$suffix}.js", __FILE__), array('jquery', ),
            filemtime( plugin_dir_path( __FILE__ ) . "/assets/main{$suffix}.js" ), true);
    wp_enqueue_script( 'simple_feedback' );

    if (is_admin() && ($opts = orbisius_simple_feedback_get_options()) && !empty($opts['show_in_admin'])) {
...
function orbisius_simple_feedback_inject_feedback() {
    ...
    $json_str = json_encode( array( 
        'trigger_action' => $opts['trigger_action'],
    ) );

	$form_buff = <<<FORM_EOF
<script>
    var orbisius_simple_feedback_container_cfg = $json_str;
</script>
<div class="orbisius_simple_feedback_container">
    <div class="feedback_wrapper feedback_wrapper_short feedback_wrapper_$call_to_action_alignment" onmouseover="try { orbisius_simple_feedback_setup_js(); } catch (e) {} ">
        <div class="feedback_title_wrapper">
            ...
            <a href="javascript:void(0);" class='close_button_link hide'><span __class='close_button'>X</span> Close</a>
        </div>
		<div class="feedback hide">
			<form id="orbisius_beta_feedback_form" class="orbisius_beta_feedback_form">
				...
				<input type="submit" id="orbisius_beta_feedback_form_submit" name="send" value="Send Feedback" />
                            ...
                            <div class="inside">
                                <form method="post" action="options.php">
                                    ...
                                    <table class="form-table">
                                        <tr>
                                            <th scope="row">Call to Action (Title)</th>
                                            <td>
                                        ...
                                        <tr valign="top">
                                            <th scope="row">What should trigger the box?</th>
                                            <td>
                                                <label for="radio1">
                                                    <input type="radio" id="radio1" name="orbisius_simple_feedback_options[trigger_action]"
                                                        value="mouseenter" <?php echo empty($opts['trigger_action']) ? '' : 'checked="checked"'; ?> /> Mouse Over
                                                    <br/>So as soon as the user's cursor reaches the Feedback title the box will show up.
                                                </label>
                                                <br/>
                                                <label for="radio2">
                                                    <input type="radio" id="radio2" name="orbisius_simple_feedback_options[trigger_action]"
                                                        value="click" <?php echo!empty($opts['trigger_action']) ? '' : 'checked="checked"'; ?> /> Click
                                                </label>
                                            </td>
                                        </tr>
                                        <tr>
